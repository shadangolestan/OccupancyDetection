...
for i in range(room_data.shape[0]):
    ...
    if int(room_data[i][Hour]) < start_hour or int(room_data[i][Hour]) >= end_hour:
        continue
    co2_hist.append(room_data[i][CO2])

#print(room_data
#room_1_co2_hist_density, room_1_co2_hist_edges = np.histogram(a=room_1_co2, bins='auto', density=True)
#print(room_1_co2_hist_density)
if Plot_Gamma == True:
    fit_alpha, fit_loc, fit_beta = stats.gamma.fit(co2_hist)
if Plot_GMM == True:
    GMM = mix.GaussianMixture(n_components=GMM_components)
    x = np.asarray(co2_hist).reshape(-1,1)
    GMM.fit(x)
    ...
    for i in range(len(co2_hist)):
        ...
        for j in range(GMM.weights_.shape[0]):
            sum+=GMM.weights_[j]*stats.norm.pdf(co2_hist[i], loc=GMM.means_[j][0], scale=GMM.covariances_[j][0][0])
        GMM_pdf.append(sum)

#print(GMM_pdf)
plt.hist(x=co2_hist, bins='auto', histtype='step', density=True, label='Histogram')
if Plot_GMM == True:
    plt.plot(co2_hist,GMM_pdf,'g.', label=('GMM-'+str(GMM.weights_.shape[0])))
if Plot_Gamma == True:
    plt.plot(co2_hist, stats.gamma.pdf(x=co2_hist, a=fit_alpha, loc=fit_loc, scale=fit_beta), 'r.', label='Gamma')
